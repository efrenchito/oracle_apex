...
BEGIN
      ...
      --Transform XMLTYPE to BLOB
        l_xml_blob := utl_raw.cast_to_raw(l_xml_type.getClobVal()); 
        --INSERT INTO test_xml (xml_blob) VALUES (l_xml_blob);
          -->Generate ZIP_FILE from BLOB
        APEX_ZIP.ADD_FILE(p_zipped_blob => l_zipped_file
                         ,p_file_name   => 'XMLWebServiceDian.zip'
        ...
        FOR i IN 1 .. l_files.count LOOP
            ...
            -->Export ZIP_FILE to PATH(UTL_FILE)
            ...
            --dbms_output.put_line(l_zipped_file.);
        END LOOP;

        -->Export ZIP_FILE to PATH(UTL_FILE)
        /*l_file := UTL_FILE.fopen('MY_DIR','XMLWebServiceDian.ZIP','wb', 32767);
