CREATE OR REPLACE PACKAGE gfe_xml IS
  ...
  -- Purpose : 
  FUNCTION getBlobFromClob(p_clob IN CLOB) RETURN BLOB;
  ...
  FUNCTION getDocumentLine( p_xmldoc IN xmldom.DOMDocument
                         , p_root_node IN xmldom.DOMNode
                         , p_document_id IN NUMBER) RETURN xmldom.DOMNode;

  PROCEDURE getXMLDocument( p_xmldoc IN xmldom.DOMDocument
                          , p_root_node IN xmldom.DOMNode
...
CREATE OR REPLACE PACKAGE BODY gfe_xml IS
  ...
  FUNCTION getBlobFromClob(p_clob IN CLOB) RETURN BLOB IS
    v_blob BLOB;
  ...
  FUNCTION getDocumentTag(p_document_id IN NUMBER) RETURN VARCHAR2 IS
    x_document_type gfe_documents.document_type%TYPE;
    x_document_tag  gfe_documents.document_type%TYPE;
  BEGIN

    BEGIN
      SELECT gd.document_type
        INTO x_document_type
        FROM gfe_documents gd
    ...
    CASE WHEN x_document_type = 'INV' THEN x_document_tag := 'Invoice'; 
         WHEN x_document_type = 'CM'  THEN x_document_tag := 'CreditNote';
         WHEN x_document_type = 'DM'  THEN x_document_tag := 'DebitNote';
      END CASE;
    RETURN x_document_tag;
  ...
  FUNCTION getDocumentLine( p_xmldoc IN xmldom.DOMDocument
    ...
    tag_document_type gfe_documents.document_type%TYPE;
  ...
  BEGIN
    ...
    tag_document_type := getDocumentTag(p_document_id);

    --[1..*]fe:InvoiceLine>>>
    FOR r_InvoiceLine IN c_InvoiceLine(p_document_id) LOOP
      item_elmt := xmldom.createElement(p_xmldoc, 'fe:'||tag_document_type||'Line');
      user_node := xmldom.appendChild(p_root_node, xmldom.makeNode(item_elmt));
        --[1..1]cbc:ID
        /*13.1.1.1 - Numero de Linea: Numero de Linea*/
        user_node_sub1 := getTextOnlyElement(p_xmldoc => p_xmldoc
                                            ,p_root_node => user_node
                                            ,p_element_name => 'cbc:ID'
        ...
        /*13.1.1.9 - Cantidad: Cantidad del articulo solicitado. Numero de unidades servidas/prestadas.*/
        user_node_sub1 := getTextOnlyElement(p_xmldoc => p_xmldoc
                                            ,p_root_node => user_node
        ...
        user_node_sub1 := xmldom.appendChild(user_node, xmldom.makeNode(item_elmt_sub1));
          --cbc:Description
          /*13.1.1.3 - Descripcion: Descripciin del articulo que pertenece a la linea de la factura*/
          user_node_sub2 := getTextOnlyElement(p_xmldoc => p_xmldoc
                                              ,p_root_node => user_node_sub1
  ...
  BEGIN
    ...
    root_elmt := xmldom.createElement(xmldoc, 'fe:'||tag_document_type);
    ...
    /*cbc:IssueTime*/      user_node := xmldom.appendChild(root_node, gfe_xml.getTextOnlyElement(p_xmldoc => xmldoc
                                                                                                ...
                                                                                                ,x_element => item_elmt));
    IF tag_document_type = 'Invoice' THEN
    ...
    /*fe:InvoiceLine*/            user_node := xmldom.appendChild(root_node, gfe_xml.getDocumentLine(p_xmldoc => xmldoc, p_root_node => root_node, p_document_id => p_document_id));
    --
